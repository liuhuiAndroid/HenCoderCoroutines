...
fun main() = runBlocking<Unit> {
    ...
    val flow2 = flowOf(
        1, 2, 3, "rengwuxian.com", "扔物线", listOf("A", "B"), listOf(1, 2)
    )
    scope.launch {
        flow1.filterNotNull().filter { it % 2 == 0 }.collect { println("1: $it") }
        flow1.filterNotNull().filterNot { it % 2 == 0 }.collect { println("2: $it") }
        flow2.filterIsInstance<String>().collect { println("-: $it") } // 优先 // reified
        flow2.filterIsInstance<List<String>>().collect { println("3: $it") } // 类型擦除，这是个错误写法
